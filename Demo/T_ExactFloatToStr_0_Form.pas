...
begin
	if ShowDebug_ck.Checked then
		ExactFloatToStr_JH0.LogFmtX := LogFmt
	else
		ExactFloatToStr_JH0.LogFmtX := nil;
...
procedure StrToFloatProc(const Str: string; out Value: Extended);
var
...
begin
	...
	Memo1.Lines.Add('');
	try
		StrToFloatProc(Edit1.Text, {out}ext);
		TestNumber(ext);
	finally
...
begin
	...
	begin
		...
		Memo1.Lines.Add('  ' + ParseFloat(ext) + ' ' + ParseFloat(dbl) + ' ' + ParseFloat(sgl));
	end{i-loop};
end;
